...
const jsonPath = resolve(__dirname, "more-hero-images.json");
...
for (const entry of entries) {
  ...
  if (existing.length > 0) {
    ...
    console.log(`  media exists: ${entry.modelCode}`);
  } else {
    ...
    mediaId = inserted.id;
    console.log(`  media inserted: ${entry.modelCode} ${entry.path}`);
  }

  // Check if model_media link already exists
  ...
  console.log(`  model_media linked: ${entry.modelCode} (hero=${entry.isHero})`);
}
